...
const lcovPathField = z
  .string()
  .describe("Absolute path to the lcov.info file generated by your test suite.");

const projectRootField = z
  .string()
  .describe("Absolute path to the root directory of the project being analysed.");
...
server.tool(
    ...
    "Use this as a GPS to pinpoint exactly what code paths are missing tests.",
  {
    ...
    file_path: z
      .string()
      .describe(
        "Path to the source file as it appears in the LCOV report (or a suffix that uniquely identifies it, e.g. 'auth/auth.service.ts')."
      ),
  },
...
server.tool(
    ...
    "Provides the full picture so the agent can understand the logic around uncovered regions.",
  {
    ...
    file_path: z
      .string()
      .describe("Path as it appears in the LCOV report, or a suffix that uniquely identifies it."),
    start_line: z
      .number()
      .int()
      .positive()
  ...
  async ({ file_path, lcov_path, project_root, start_line, end_line }) => {
    try {
      const s = requireStore(lcov_path, project_root);
      const key = resolveFileKey(s.files, file_path);
      if (!key) {
          ...
          content: [
            {
              ...
              text:
                ...
                `Make sure project_root points to the directory containing your source files.`,
            },
          ],
        };
      }
